...
public class SWTRenderer extends AbstractPathOrientedRenderer implements Pageable {
     ...
     * Default constructor
     * 
     * @param uAgent the user agent that contains configuration details. This
     *            cannot be null.
     */
    ...
    private static boolean outer(BorderProps a) {
        if (a.style == Constants.EN_NONE)
            return false;
        if (a.isCollapseOuter())
            return true;
        if (BorderProps.getClippedWidth(a) == 0)
            return true;
        return false;
    }
    ...
    @Override
    protected void drawBorders(Rectangle2D.Float borderRect, BorderProps bpsTop, BorderProps bpsBottom,
            BorderProps bpsLeft, BorderProps bpsRight, Color innerBackgroundColor) {
        if (bpsTop == null)
            bpsTop = new BorderProps(Constants.EN_NONE, 0, 0, 0, BLACK, Mode.SEPARATE);
        if (bpsBottom == null)
            bpsBottom = new BorderProps(Constants.EN_NONE, 0, 0, 0, BLACK, Mode.SEPARATE);
        if (bpsLeft == null)
            bpsLeft = new BorderProps(Constants.EN_NONE, 0, 0, 0, BLACK, Mode.SEPARATE);
        if (bpsRight == null)
            bpsRight = new BorderProps(Constants.EN_NONE, 0, 0, 0, BLACK, Mode.SEPARATE);
        ...
        if (widthTop == widthBottom && widthBottom == widthLeft && widthLeft == widthRight && widthRight == widthTop &&
                ...
                ((bpsTop.style != Constants.EN_DASHED && bpsTop.style != Constants.EN_DOTTED)
                        || (outer(bpsTop) && outer(bpsBottom) && outer(bpsLeft) && outer(bpsRight)))
                &&
                bpsTop.color.equals(bpsBottom.color) && bpsBottom.color.equals(bpsLeft.color) && bpsLeft.color.equals(bpsRight.color)
                && bpsRight.color.equals(bpsTop.color)) {
            drawRectangle(middle.x, middle.y, middle.w, middle.h, bpsTop, false);
        } else {
            Rect outer = new Rect(middle.x - widthLeft / 2, middle.y - widthTop / 2, middle.w + widthLeft / 2 + widthRight / 2,
                    middle.h + widthTop / 2 + widthBottom / 2);
            Rect inner = new Rect(middle.x + widthLeft / 2, middle.y + widthTop / 2, middle.w - widthLeft / 2 - widthRight / 2,
                    middle.h - widthTop / 2 - widthBottom / 2);
    ...
    protected void renderInlineParent(InlineParent ip) {
        ...
        textOutput.text(concat.toString(), rx / 1000f, bl / 1000f, tf.getFontName(), font.getFontSize(), Convert.toRGBA(col), null, null, null);
    ...
    public void renderLeader(Leader area) {
        ...
        case EN_RIDGE:
        case EN_GROOVE:
            props = new BorderProps(area.getRuleStyle() == EN_GROOVE ? Constants.EN_INSET : Constants.EN_OUTSET, area.getRuleThickness() / 4, 0, 0, col,
                    Mode.SEPARATE);
