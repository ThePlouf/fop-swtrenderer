...
public class GeometryTest {
            ...
            while (line != null) {
                ...
                if (line.length() > 0 && !line.startsWith("#")) {
                    ...
                    case PathIterator.SEG_QUADTO:
                        ...
                        break;
                    case PathIterator.SEG_CUBICTO:
                        ans.curveTo(Double.parseDouble(parts[1]), Double.parseDouble(parts[2]), Double.parseDouble(parts[3]), Double.parseDouble(parts[4]),
    ...
    private static void genData(String text, String name) throws IOException {
        ...
        double offset = 20;
        save(Geometry.offsetShape(textShape, offset, BasicStroke.JOIN_BEVEL), Paths.get("src/test/resources/be/pdty/fop/offset/" + name + "_bevel.txt"),
                "Offset " + offset + " Bevel geometry for " + name + ": " + text);
        save(Geometry.offsetShape(textShape, offset, BasicStroke.JOIN_MITER), Paths.get("src/test/resources/be/pdty/fop/offset/" + name + "_miter.txt"),
                "Offset " + offset + " Miter geometry for " + name + ": " + text);
        save(Geometry.offsetShape(textShape, offset, BasicStroke.JOIN_ROUND), Paths.get("src/test/resources/be/pdty/fop/offset/" + name + "_round.txt"),
                "Offset " + offset + " Round geometry for " + name + ": " + text);
    }
